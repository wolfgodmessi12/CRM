...
# app/controllers/integrations/fieldroutes/v1/events_controller.rb
module Integrations
  module Fieldroutes
    module V1
      class EventsController < Fieldroutes::V1::IntegrationsController
        # (DELETE) delete a FieldRoutes event
        # /integrations/fieldroutes/v1/events/:id
        # integrations_fieldroutes_v1_event_path(:id)
        # integrations_fieldroutes_v1_event_url(:id)
        def destroy
          @client_api_integration_events.events.delete(params[:id].to_s)
          @client_api_integration_events.save
        end

        # (GET) edit a FieldRoutes event
        # /integrations/fieldroutes/v1/events/:id/edit
        # edit_integrations_fieldroutes_v1_event_path(:id)
        # edit_integrations_fieldroutes_v1_event_url(:id)
        def edit
          @event_id = params.dig(:id)
        end

        # (GET) list FieldRoutes events
        # /integrations/fieldroutes/v1/events
        # integrations_fieldroutes_v1_events_path
        # integrations_fieldroutes_v1_events_url
        def index; end

        # (GET) initialize a new FieldRoutes event
        # /integrations/fieldroutes/v1/events/new
        # new_integrations_fieldroutes_v1_event_path
        # new_integrations_fieldroutes_v1_event_url
        def new
          @event_id = SecureRandom.uuid
          @client_api_integration_events.update(events: @client_api_integration_events.events.merge({ @event_id => default_event }))
        end

        # (GET) refresh FieldRoutes employees
        # /integrations/fieldroutes/v1/events/refresh_employees/:id
        # integrations_fieldroutes_v1_event_refresh_employees_path(:id)
        # integrations_fieldroutes_v1_event_refresh_employees_url(:id)
        def refresh_employees
          @event_id = params.dig(:id)
          Integration::Fieldroutes::V1::Base.new(@client_api_integration).refresh_employees
        end

        # (PUT/PATCH) update a FieldRoutes event
        # /integrations/fieldroutes/v1/events/:id
        # integrations_fieldroutes_v1_event_path(:id)
        # integrations_fieldroutes_v1_event_url(:id)
        def update
        ...
        def params_update_events
          ...
          sanitized_params = params.require(:event).permit(:event_type, :assign_contact_to_user, :campaign_id, :event_new, :event_updated, :group_id, :id, :range_max, :stage_id, :start_date_updated, :tag_id, :tech_updated, :total, :total_due, ext_tech_ids: [], stop_campaign_ids: [])
