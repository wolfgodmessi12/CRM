...
# app/presenters/integrations/thumbtack/v2/presenter.rb
module Integrations
  module Thumbtack
    module V2
      class Presenter < BasePresenter
        ...
        # return URL used to request authorization code from Thumbtack
        def connect_to_thumbtack_url
          Integration::Thumbtack::V2::Base.new(@client_api_integration).connect_to_thumbtack_url
        end

        def customer_type
          @event.dig(:criteria, :customer_type) || []
        end
        ...
        def event_types_array
          Integration::Thumbtack::V2::Base.new(@client_api_integration).lead_types
        end
        ...
        def form_url
          @event_id.present? ? Rails.application.routes.url_helpers.integrations_thumbtack_v2_event_path(@event_id) : Rails.application.routes.url_helpers.integrations_thumbtack_v2_events_path
        end
        ...
        # verify that Thumbtack credentials are valid
        # presenter.valid_credentials?
        def valid_credentials?
          Integration::Thumbtack::V2::Base.new(@client_api_integration).valid_credentials?
        end
      end
    end
  end
