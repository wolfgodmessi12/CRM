...
module Integrations
  module Angi
    module V1
      module Events
        class Presenter < Integrations::Angi::V1::Presenter
          attr_accessor :event
          attr_reader :client_api_integration_events, :event_id

          # Integrations::Angi::V1::Events::Presenter.new(client_api_integration: @client_api_integration)
          #   (req) client_api_integration: (ClientApiIntegration) or (Integer)

          def initialize(args = {})
            super

            @client_api_integration_events = @client.client_api_integrations.find_by(target: 'angi', name: 'events')
            @event                         = nil
            @event_id                      = nil
          end
          ...
          def event_name
            @event&.dig(:criteria, :name).to_s
          end

          def event_type_name
            Integration::Angi::V1::Base::EVENT_TYPE_OPTIONS.find { |et| et.second == event.dig(:criteria, :event_type) }&.first.to_s
          end
