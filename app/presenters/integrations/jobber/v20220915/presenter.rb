...
# app/presenters/integrations/jobber/v20220915/presenter.rb
module Integrations
  module Jobber
    module V20220915
      class Presenter < BasePresenter
        ...
        # Integrations::Jobber::V20220915::Presenter.new(client_api_integration: @client_api_integration)
        ...
        def connect_to_jobber_url
          Integration::Jobber::V20220915::Base.new(@client_api_integration).connect_to_jobber_url
        end

        def contact_imports_remaining_string(user_id)
          Integration::Jobber::V20220915::Base.new(@client_api_integration).import_contacts_remaining_string(user_id)
        end
        ...
        def form_url
          @webhook_id.present? ? Rails.application.routes.url_helpers.integrations_jobber_v20220915_webhook_path(@webhook_id, event: @event_name) : Rails.application.routes.url_helpers.integrations_jobber_v20220915_webhooks_path
        end
        ...
        def status_for_select(event_name = @event_name)
          ...
          when 'invoice'
            Integration::Jobber::V20220915::Base::INVOICE_STATUSES
          when 'job'
            Integration::Jobber::V20220915::Base::JOB_STATUSES
          when 'quote'
            Integration::Jobber::V20220915::Base::QUOTE_STATUSES
          when 'request'
            Integration::Jobber::V20220915::Base::REQUEST_STATUSES
          when 'visit'
            Integration::Jobber::V20220915::Base::VISIT_STATUSES
          else
            []
          end
        end
        ...
        def valid_credentials?
          Integration::Jobber::V20220915::Base.new(@client_api_integration).valid_credentials?
        end
        ...
        def webhook_events_array
          Integration::Jobber::V20220915::Base::EVENTS.map { |webhook| [webhook[:name].titleize, webhook[:event].tr('.', '_')] }
        end
        ...
        def webhook_name
          (Integration::Jobber::V20220915::Base::EVENTS.find { |event| event[:event] == @event_name } || { name: 'unknown' }).dig(:name).to_s.titleize
        end
