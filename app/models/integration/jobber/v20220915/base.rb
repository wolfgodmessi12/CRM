  ...
  module Jobber
    module V20220915
      class Base < Integration::Jobber::Base
        include Integration::Jobber::V20220915::ImportContacts
        ...
        # client_id = xx
        # client_api_integration = ClientApiIntegration.find_by(client_id: client_id, target: 'jobber', name: ''); jb_model = Integration::Jobber::V20220915::Base.new(client_api_integration); jb_model.valid_credentials?; jb_client = Integrations::JobBer::V20220915::Base.new(client_api_integration.credentials)

        # jb_model = Integration::Jobber::V20220915::Base.new()
        ...
        def connect_to_jobber_url
          ...
          "https://secure.getjobber.com/api/oauth/authorize?client_id=#{Rails.application.credentials[:jobber][:client_id]}&redirect_uri=#{Rails.application.routes.url_helpers.integrations_jobber_v20220915_auth_code_url(host: I18n.with_locale(@client_api_integration.client.tenant) { I18n.t("tenant.#{Rails.env}.app_host") })}&state=#{@client_api_integration.auth_code}"
        end
        ...
        def create_or_update_client(contact)
          ...
          if (jobber_client = contact.ext_references.find_by(target: 'jobber'))
            @jb_client.client_update(jobber_client.ext_id, contact_attrs)
          else
            @jb_client.client_create(contact_attrs)
          end

          return unless @jb_client.success? && (contact_ext_reference = contact.ext_references.find_or_create_by(target: 'jobber'))

          contact_ext_reference.update(ext_id: @jb_client.result)
        end

        # delete Jobber credentials
        # jb_model.delete_credentials
        def delete_credentials
          JsonLog.info 'Integration::Jobber::V20220915::Base.delete_credentials', { client_api_integration: @client_api_integration&.attributes_cleaned }
        ...
        def disconnect_account
          JsonLog.info 'Integration::Jobber::V20220915::Base.disconnect_account', { client_api_integration: @client_api_integration&.attributes_cleaned }
          self.valid_credentials?
          jb_client = Integrations::JobBer::V20220915::Base.new(@client_api_integration.credentials)
          jb_client.disconnect_account

          self.delete_credentials if jb_client.success?
        ...
        def update_account
          JsonLog.info 'Integration::Jobber::V20220915::Base.update_account', { client_api_integration: @client_api_integration&.attributes_cleaned }
          self.valid_credentials?
          @client_api_integration.update(account: Integrations::JobBer::V20220915::Base.new(@client_api_integration.credentials).account)
        end
        ...
        def update_credentials(code = nil)
          if code.present?
            self.update_credentials_by_code(code)
          elsif @client_api_integration.credentials.dig('refresh_token').present?
            self.update_credentials_by_refresh_token
          end
        ...
        def update_credentials_by_code(code)
          @client_api_integration.credentials = Integrations::JobBer::V20220915::Base.new(@client_api_integration.credentials).request_access_token(code)
          @jb_client = Integrations::JobBer::V20220915::Base.new(@client_api_integration.credentials)
        ...
        def update_credentials_by_refresh_token
          @client_api_integration.credentials = Integrations::JobBer::V20220915::Base.new(@client_api_integration.credentials).refresh_access_token(@client_api_integration.credentials['refresh_token'])
          @jb_client = Integrations::JobBer::V20220915::Base.new(@client_api_integration.credentials)
          self.update_credentials_expiration_and_version
        end
        ...
        def update_credentials_expiration_and_version
          if @client_api_integration.credentials.dig('access_token').present? && @client_api_integration.credentials.dig('refresh_token').present?
            ...
            @client_api_integration.credentials[:version]    = '20220915'
        ...
        def valid_credentials?
          if @client_api_integration.credentials.present? && Integrations::JobBer::V20220915::Base.new(@client_api_integration.credentials).valid_credentials?
            true
