...
# app/models/integration/jobber/v20231115/event.rb
module Integration
  module Jobber
    module V20231115
      class Event < Integration::Jobber::Base
        ...
        # jb_model = Integration::Jobber::V20231115::Event.new(data); Integration::Jobber::V20231115::Base.new(client_api_integration).valid_credentials?; jb_client = Integrations::Jobber::V20231115::Base.new(client_api_integration).credentials)

        # initialize Jobber event
        # jb_event_client = Integration::Jobber::V20231115::Event.new()
        ...
        def process
          Rails.logger.info "Integration::Jobber::V20231115::Event.process: #{{ client_id: @client_api_integration&.client_id, contact_id: @contact&.id, account_id: @account_id, item_id: @item_id, event_object: @event_object, event_action: @event_action, client_api_integration: @client_api_integration&.attributes_cleaned }.inspect} - File: #{__FILE__} - Line: #{__LINE__} - Calling: { File: #{caller_locations(1..1).first.path} - Line: #{caller_locations(1..1).first.lineno} }"
          @result = false
          ...
          when 'app'
            @jb_model.delete_credentials if @event_action == 'disconnect'
          end
        ...
        def create_or_update_contact(jobber_client_id)
          Rails.logger.info "Integration::Jobber::V20231115::Event.create_or_update_contact: #{{ client_id: @client_api_integration&.client_id, contact_id: @contact&.id, jobber_client_id: }.inspect} - File: #{__FILE__} - Line: #{__LINE__} - Calling: { File: #{caller_locations(1..1).first.path} - Line: #{caller_locations(1..1).first.lineno} }"
          return false unless self.ok_to_process && jobber_client_id.present? &&
                              @jb_model.valid_credentials? && (jb_client = Integrations::JobBer::V20231115::Base.new(@client_api_integration.credentials)) &&
          ...
          if @contact.valid?
            ...
            Rails.logger.info "Integration::Jobber::V20231115::Event.create_or_update_contact: #{{ client_id: @client_api_integration.client_id, contact_id: @contact.id, errors: @contact.errors.full_messages, contact_phones: @contact.contact_phones }.inspect} - File: #{__FILE__} - Line: #{__LINE__} - Calling: { File: #{caller_locations(1..1).first.path} - Line: #{caller_locations(1..1).first.lineno} }"
            false
          end
        end
        ...
        def create_or_update_estimate(jobber_quote_id)
          Rails.logger.info "Integration::Jobber::V20231115::Event.create_or_update_estimate: #{{ client_id: @client_api_integration&.client_id, contact_id: @contact&.id, jobber_quote_id:, event_action: @event_action }.inspect} - File: #{__FILE__} - Line: #{__LINE__} - Calling: { File: #{caller_locations(1..1).first.path} - Line: #{caller_locations(1..1).first.lineno} }"
          ...
          if @contact_estimate.valid?
            ...
            Rails.logger.info "Integration::Jobber::V20231115::Event.create_or_update_estimate: #{{ client_id: @client.id, contact_estimate_errors: @contact_estimate.errors.full_messages.join(' '), contact_estimate: @contact_estimate }.inspect} - File: #{__FILE__} - Line: #{__LINE__} - Calling: { File: #{caller_locations(1..1).first.path} - Line: #{caller_locations(1..1).first.lineno} }"
          end
        ...
        def create_or_update_invoice(jobber_invoice_id)
          Rails.logger.info "Integration::Jobber::V20231115::Event.create_or_update_invoice: #{{ client_id: @client_api_integration&.client_id, contact_id: @contact&.id, jobber_invoice_id:, event_action: @event_action }.inspect} - File: #{__FILE__} - Line: #{__LINE__} - Calling: { File: #{caller_locations(1..1).first.path} - Line: #{caller_locations(1..1).first.lineno} }"
          ...
          if @contact_invoice.valid?
            ...
            Rails.logger.info "Integration::Jobber::V20231115::Event.create_or_update_invoice: #{{ client_id: @client_api_integration&.client_id, contact_invoice_errors: @contact_invoice.errors.full_messages.join(' '), contact_invoice: @contact_invoice }.inspect} - File: #{__FILE__} - Line: #{__LINE__} - Calling: { File: #{caller_locations(1..1).first.path} - Line: #{caller_locations(1..1).first.lineno} }"
          end
        ...
        def create_or_update_job(jobber_job_id)
          Rails.logger.info "Integration::Jobber::V20231115::Event.create_or_update_job: #{{ client_id: @client_api_integration&.client_id, contact_id: @contact&.id, jobber_job_id:, event_action: @event_action }.inspect} - File: #{__FILE__} - Line: #{__LINE__} - Calling: { File: #{caller_locations(1..1).first.path} - Line: #{caller_locations(1..1).first.lineno} }"
          ...
          @contact_job.update(
            ...
            description:                       (jb_client.result.dig(:title) || @contact_job.description).to_s,
          ...
          if @contact_job.valid?
            ...
            Rails.logger.info "Integration::Jobber::V20231115::Event.create_or_update_job: #{{ client_id: @client.id, contact_job_errors: @contact_job.errors.full_messages.join(' '), contact_job: @contact_job }.inspect} - File: #{__FILE__} - Line: #{__LINE__} - Calling: { File: #{caller_locations(1..1).first.path} - Line: #{caller_locations(1..1).first.lineno} }"
          end
        ...
        def create_or_update_lineitems(job_or_estimate, jobber_lineitems)
          jobber_lineitems&.each do |line_item|
            job_or_estimate.lineitems.find_or_create_by(ext_id: line_item.dig(:linkedProductOrService, :id) || '', name: line_item.dig(:linkedProductOrService, :name).to_s, total: line_item.dig(:totalPrice).to_d)
          end
        end
        ...
        def create_or_update_request(jobber_request_id)
          Rails.logger.info "Integration::Jobber::V20231115::Event.create_or_update_request: #{{ client_id: @client_api_integration&.client_id, contact_id: @contact&.id, jobber_request_id:, event_action: @event_action }.inspect} - File: #{__FILE__} - Line: #{__LINE__} - Calling: { File: #{caller_locations(1..1).first.path} - Line: #{caller_locations(1..1).first.lineno} }"
        ...
        def create_or_update_visit(jobber_visit_id)
          Rails.logger.info "Integration::Jobber::V20231115::Event.create_or_update_visit: #{{ client_id: @client_api_integration&.client_id, contact_id: @contact&.id, jobber_visit_id:, event_action: @event_action }.inspect} - File: #{__FILE__} - Line: #{__LINE__} - Calling: { File: #{caller_locations(1..1).first.path} - Line: #{caller_locations(1..1).first.lineno} }"
        ...
        def destroy_deleted_lineitems(job_or_estimate, jobber_lineitems)
          (job_or_estimate.lineitems.pluck(:ext_id, :name) - jobber_lineitems.filter_map { |lineitem| [lineitem.dig(:linkedProductOrService, :id) || '', lineitem.dig(:linkedProductOrService, :name).to_s] }).each do |li|
        ...
        def process_actions_for_event
          Rails.logger.info "Integration::Jobber::V20231115::Event.process_actions_for_event: #{{ client_id: @client_api_integration&.client_id, contact_id: @contact&.id, event_action: @event_action, event_new: @event_new, event_object: @event_object, start_date_updated: @start_date_updated, tech_updated: @tech_updated, contact_job: @contact_job, contact_estimate: @contact_estimate, contact_visit: @contact_visit }.inspect} - File: #{__FILE__} - Line: #{__LINE__} - Calling: { File: #{caller_locations(1..1).first.path} - Line: #{caller_locations(1..1).first.lineno} }"
        ...
        def not_event_line_items_match?(event)
          if %w[quote].include?(@event_object)
            event.dig(:criteria, :line_items).present? && !event.dig(:criteria, :line_items).intersect?(@contact_estimate&.lineitems&.pluck(:ext_id).to_a)
          elsif %w[job visit].include?(@event_object)
            event.dig(:criteria, :line_items).present? && !event.dig(:criteria, :line_items).intersect?(@contact_job&.lineitems&.pluck(:ext_id).to_a)
          elsif %w[invoice].include?(@event_object)
            event.dig(:criteria, :line_items).present? && !event.dig(:criteria, :line_items).intersect?(@contact_invoice&.lineitems&.pluck(:ext_id).to_a)
          else
            false
          end
        end
