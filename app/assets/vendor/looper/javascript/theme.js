class Theme {
  constructor() {
    ...
    // current skin
    this.skin = Cookies.get('skin') || 'default'
    // this.skin = localStorage.getItem('skin') || 'default'

    // current aside menu mode
    this.hasCompactMenu = JSON.parse(Cookies.get('hasCompactMenu') || 'false')
  ...
  nestedDropdown() {
    $('.dropdown-menu [data-toggle="dropdown"]').on('click', function (e) {
      ...
      $trigger
        .parents('.nav-item')
        .on('hidden.bs.dropdown', () => $dropdownMenu.removeClass('show'))
    })
  }
  ...
  modalScrollable() {
    ...
    $('.modal-dialog-scrollable .modal-body, .modal-drawer .modal-body').off('scroll')
  ...
  autofocusInputBehaviour() {
    $(document).off('shown.bs.modal shown.bs.dropdown', '.modal, .dropdown')
  ...
  asideMenu() {
    ...
    if (window.StackedMenu && this.isExists('#stacked-menu')) {
      ...
      $(this.stackedMenu.selector).on('menu:open menu:close', () => {
  ...
  collapseAsideMenu() {
    if (typeof this.stackedMenu !== 'undefined') {
      this.stackedMenu
        ...
        .hoverable(true)
      // update aside mode
      this.hasCompactMenu = true
      // as well as localStorage data
      Cookies.set('hasCompactMenu', true, { expires: 7 })
  ...
  expandAsideMenu() {
    if (typeof this.stackedMenu !== 'undefined') {
      this.stackedMenu
        ...
        .hoverable(false)
      // update aside mode
      this.hasCompactMenu = false
      // as well as localStorage data
      Cookies.set('hasCompactMenu', false, { expires: 7 })
  ...
  handleAsideMenu() {
    $('body')
      .off('click', '[data-toggle="aside-collapse"]')
      .off('click', '[data-toggle="aside-expand"]')
      .off('click', '[data-toggle="aside-menu"]')
    $('body')
      .on('click', '[data-toggle="aside-collapse"]', (e) => {
        e.preventDefault()
        this.collapseAsideMenu()
      })
      .on('click', '[data-toggle="aside-expand"]', (e) => {
        e.preventDefault()
        this.expandAsideMenu()
      })
      .on('click', '[data-toggle="aside-menu"]', (e) => {
        e.preventDefault()
  ...
  sidebar() {
    ...
    $(document).off('click', '[data-toggle="sidebar"], .sidebar-backdrop')
   ...
   */
  hamburger() {
    $(document).off('click', '.hamburger-toggle')
    ...
    if (window.Chart) {
      ...
      const settings = {
        global: {
          ...
          fontFamily: '"Hanken Grotesk", -apple-system, BlinkMacSystemFont, "Helvetica Neue", "Apple Color Emoji", sans-serif',
          tooltips: {
  ...
  zxcvbn() {
    if (window.zxcvbn) {
      $('.form-strength-meter').each(function () {
        ...
        const strength = ['bg-red', 'bg-orange', 'bg-yellow', 'bg-teal', 'bg-indigo']
        ...
        $(input).on('keyup', function () {
          ...
          if (val !== '') {
            ...
            $(feedback).html(`<strong>${result.feedback.warning}</strong> ${result.feedback.suggestions}`)
          } else {
  ...
  eventProps() {
    $(document)
      .off('click', '.stop-propagation')
      .off('click', '.prevent-default');
    $(document)
      .on('click', '.stop-propagation', (e) => {
        e.stopPropagation()
      })
      .on('click', '.prevent-default', (e) => {
        e.preventDefault()
      })
  }
...
// Initialize Theme as Looper so we can use it as global variable.
// @example
// Looper.setSkin('dark')
