...
# app/lib/integrations/jot_form/v1/base.rb
module Integrations
  module JotForm
    module V1
      class Base
        # initialize JotForm
        # jf_client = Integrations::JotForm::V1::Base.new()
        #   (req) api_key:       (String)
        #   (req) jotform_forms: (Hash)
        def initialize(api_key, jotform_forms)
          @jf_client     = Integrations::JotForm::Jotform.new(api_key)
        ...
        def forms
          ...
          begin
            ...
            (@jf_client.getForms || {}).each do |form|
              ...
              jotform_form['campaign_id'] = @jotform_forms.dig(form['id'].to_s, 'campaign_id').to_i
              jotform_form['questions'] = {}

              (@jf_client.getFormQuestions(form['id']) || {}).each_value do |question|
                unless controls_01.include?(question['type'])
                  ...
                  if controls_02.include?(question.dig('type'))
                    ...
                    jotform_form['questions'][question['qid']] = {
                      ...
                      'label'           => question['text'],
                      'custom_field_id' => @jotform_forms.dig(form['id'].to_s, 'questions', question['qid'].to_s, 'custom_field_id').to_s,
                      'options'         => {},
                      'sublabels'       => {},
                    ...
                    (question.dig('options') || '').split('|').each do |option|
                      jotform_options[option] = @jotform_forms.dig(form['id'], 'questions', question['qid'], 'options', option).to_i
                    end
                    ...
                    (question.dig('sublabels') || {}).each do |var_name, label|
                      unless masked_full.include?(var_name) || label.empty?
                        jotform_sublabels[var_name] = {
                          'label'           => label,
                          'custom_field_id' => @jotform_forms.dig(form['id'], 'questions', question['qid'], 'sublabels', var_name, 'custom_field_id').to_s
                        }
                      end
                    end
            ...
            ProcessError::Report.send(
              error_message: "IntegrationsController::GetJotformForms: #{e.message}",
              variables:     {
                e:            e.inspect,
                e_message:    e.message,
                response:     response.inspect,
                jotform_form: defined?(jotform_form) ? jotform_form.inspect : nil,
                form:         defined?(form) ? form.inspect : nil,
                question_id:  defined?(question_id) ? question_id.inspect : nil,
                question:     defined?(question) ? question.inspect : nil,
                option:       defined?(option) ? option.inspect : nil,
                name:         defined?(name) ? name.inspect : nil,
                label:        defined?(label) ? label.inspect : nil,
                file:         __FILE__,
                line:         __LINE__
              }
            )
        ...
        def form_webhooks(form_id)
          @jf_client.getFormWebhooks(form_id)
        end
      end
    end
  end
